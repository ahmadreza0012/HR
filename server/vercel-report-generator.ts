...
export default async function handler(req: Request, res: Response) {
  ...
  try {
    ...
    await scriptFetch("reports/register", { method: "POST", headers: { "Content-Type": "text/plain;charset=utf-8" }, body: JSON.stringify({ id: reportId, reportNumber: `RPT-${Date.now()}`, employeeId, employeeName: snapshot.employee.fullName, rangeStart: start, rangeEnd: end, reportType: type, fileName, mimeType: isXlsx ? "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" : "application/pdf", fileBase64: file.toString("base64"), pdfHash: digest, payloadHash: hash(Buffer.from(JSON.stringify(snapshot))), issuedAt: new Date().toISOString(), isDemo: Boolean(snapshot.employee.isDemo), snapshot }) });
